...
;;
 ...
 ;;   
;;
;;f you see something like
;;
 ;;   /usr/bin/javac
 ;;   
;;
;;hen you’re good to go. If you don’t see any output from that command, install the appropriate [Oracle JDK 1.7](http://www.oracle.com/technetwork/java/javase/downloads/index.html) for your operating system, or whatever JDK your package manager has available.
;;
;;hen you have a JDK, you’ll need [Leiningen](http://leiningen.org), the Clojure build tool. If you’re on a Linux or OS X computer, the instructions below should get you going right away. If you’re on Windows, see the Leiningen page for an installer. If you get stuck, you might want to start with a [primer on command line basics](http://blog.teamtreehouse.com/command-line-basics).
;;
 ;;   mkdir -p ~/bin
 ;;   cd ~/bin
 ...
 ;;   
;;
;;einingen automatically handles installing Clojure, finding libraries from the internet, and building and running your programs. We’ll create a new Leiningen project to play around in:
;;
 ;;   cd
 ;;   lein new scratch
 ;;   
;;
;;his creates a new directory in your homedir, called `scratch`. If you see `command not found` instead, it means the directory `~/bin` isn’t registered with your terminal as a place to search for programs. To fix this, add the line
;;
 ;;   export PATH="$PATH":~/bin
 ;;   
;;
;;o the file `.bash_profile` in your home directory, then run `source ~/.bash_profile`. Re-running `lein new scratch` should work.
;;
;;et’s enter that directory, and start using Clojure itself:
;;
 ;;   cd scratch
 ;;   lein repl
 ;;   
...
-;;------------------------------------------------------
;;
;;hen you type `lein repl` at the terminal, you’ll see something like this:
;;
 ...
 ;;   
;;
;;his is an interactive Clojure environment called a REPL, for “Read, Evaluate, Print Loop”. It’s going to _read_ a program we enter, run that program, and print the results. REPLs give you quick feedback, so they’re a great way to explore a program interactively, run tests, and prototype new ideas.
;;
;;et’s write a simple program. The simplest, in fact. Type “nil”, and hit enter.
;;
 ;;   user=> nil
 ;;   nil
 ;;   
...
;;
 ...
 ;;   
;;
;;his is the number zero. Its numeric friends are `1`, `-47`, `1.2e-4`, `1/3`, and so on. We might also talk about _strings_, which are chunks of text surrounded by double quotes:
;;
 ;;   user=> "hi there!"
 ;;   "hi there!"
 ;;   
...
;;
 ...
 ;;   
;;
;;his is a verb called `inc`–short for “increment”. Specifically, `inc` is a _symbol_ which _points_ to a verb: `#<core$inc clojure.core$inc@6f7ef41c>`– just like the word “run” is a _name_ for the _concept_ of running.
;;
;;here’s a key distinction here–that a signifier, a reference, a label, is not the same as the signified, the referent, the concept itself. If you write the word “run” on paper, the ink means nothing by itself. It’s just a symbol. But in the mind of a reader, that symbol takes on _meaning_; the idea of running.
;;
;;nlike the number 0, or the string “hi”, symbols are references to other values. when Clojure evaluates a symbol, it looks up that symbol’s meaning. Look up `inc`, and you get `#<core$inc clojure.core$inc@6f7ef41c>`.
;;
;;an we refer to the symbol itself, _without_ looking up its meaning?
;;
 ;;   user=> 'inc
 ;;   inc
 ;;   
;;
;;es. The single quote `'` _escapes_ a sentence. In programming languages, we call sentences `expressions` or `statements`. A quote says “Rather than _evaluating_ this expression’s text, simply return the text itself, unchanged.” Quote a symbol, get a symbol. Quote a number, get a number. Quote anything, and get it back exactly as it came in.
;;
 ;;   user=> '123
 ;;   123
 ;;   user=> '"foo"
...
;;
 ...
 ;;   
;;
;;ut let’s try something simpler. Something we know how to talk about. “Increment the number zero.” As a tree:
;;
 ;;   Increment
 ;;     the number zero
 ;;   
;;
;;e have a symbol for incrementing, and we know how to write the number zero. Let’s combine them in a list:
;;
 ;;   clj=> '(inc 0)
 ;;   (inc 0)
 ;;   
;;
;; basic sentence. Remember, since it’s quoted, we’re talking about the tree, the text, the expression, by itself. Absent interpretation. If we remove the single-quote, Clojure will _interpret_ the expression:
;;
 ;;   user=> (inc 0)
 ;;   1
 ;;   
;;
;;ncrementing zero yields one. And if we wanted to increment _that_ value?
;;
 ;;   Increment
 ;;     increment
 ;;       the number zero
...
;;
 ...
 ;;   
;;
;; sentence in Lisp is a list. It starts with a verb, and is followed by zero or more objects for that verb to act on. Each part of the list can _itself_ be another list, in which case that nested list is evaluated first, just like a nested clause in a sentence. When we type
;;
 ;;   (inc (inc 0))
 ;;   
;;
;;lojure first looks up the meanings for the symbols in the code:
;;
 ;;   (#<core$inc clojure.core$inc@6f7ef41c>
 ;;     (#<core$inc clojure.core$inc@6f7ef41c>
 ;;       0))
 ;;   
;;
;;hen evaluates the innermost list `(inc 0)`, which becomes the number 1:
;;
 ;;   (#<core$inc clojure.core$inc@6f7ef41c>
 ;;    1)
 ;;   
;;
;;inally, it evaluates the outer list, incrementing the number 1:
;;
 ;;   2
 ;;   
;;
;;very list starts with a verb. Parts of a list are evaluated from left to right. Innermost lists are evaluated before outer lists.
;;
 ;;   (+ 1 (- 5 2) (+ 3 4))
 ;;   (+ 1 3       (+ 3 4))
 ;;   (+ 1 3       7)
...
;;
;;vankie on [2013-10-26](/posts/301-clojure-from-the-ground-up-welcome#comment-1678)
;;
;;oving this.
;;
...
;;
;;ooking forward to what’s next.
;;
;;[BR](https://www.gravatar.com/avatar/0f324fb00cdf26f501450a11eb821d8f?r=pg&s=96&d=identicon "BR")
;;B
;;R on [2013-10-26](/posts/301-clojure-from-the-ground-up-welcome#comment-1680)
;;
;;eriously? LISP? Haven’t we learned?
;;
;;[Peter](https://www.gravatar.com/avatar/3ad74a0b72273db84c85d67253fe5982?r=pg&s=96&d=identicon "Peter")
;;P;;eter on [2013-10-26](/posts/301-clojure-from-the-ground-up-welcome#comment-1681)
;;>;; Seriously? LISP? Haven’t we learned?
;;A;;pparently you haven’t
;;!
;;[na](https://www.gravatar.com/avatar/16fc03db97c032b24e20f13500971d42?r=pg&s=96&d=identicon "na")
;;n;;a on [2013-10-26](/posts/301-clojure-from-the-ground-up-welcome#comment-1682)
;;>;; > Seriously? LISP? Haven’t we learned>;; >;; Apparently you haven’t
;;
;;greed.
;; ! 
...
;;
;;A;;wesome! Eagerly awaiting the next installment!
;;
;;  !
...
;;
J;;D on [2013-10-27](/posts/301-clojure-from-the-ground-up-welcome#comment-1685)
;;
...
;;
S;;cientificCoder on [2013-10-27](/posts/301-clojure-from-the-ground-up-welcome#comment-1686)
;;
N;;ice ! I’ll be sure to point newcomers to your site. Kuddos and keep them coming ! ☺
;;
C;;heers.
;;
...
;;
I;;t’s not really clear who this is aimed at. For example, I got here through a feed thingie (DuckDuckGo on Android) and had no idea what exactly Clojure was until I saw the magic word “LISP”. Might be a good idea to at least link to something.
;;
!;;[Rick James](https://www.gravatar.com/avatar/601277db52ff762d09b5f29fd2257679?r=pg&s=96&d=identicon "Rick James")
;;
R;;ick James on [2013-10-27](/posts/301-clojure-from-the-ground-up-welcome#comment-1689)
;;
T;;he whole white knight paragraph was horrible. Nobody thinks you’re a great guy because you did that. You’re just an asshole.
;;
...
H;;i,
;;
G;;reat post and keep up the good work. I’d be happy to help with reviewing codes and such before you publish.
;;
J;;ust let you know we are hiring Clojure developers. Come check us out: [http://staplesinnovationlab.com/](http://staplesinnovationlab.com/)
;;
!;;[Jbeja](https://www.gravatar.com/avatar/26134de1e74c627a6df26e898229e1e0?r=pg&s=96&d=identicon "Jbeja")
;;
J;;beja on [2013-10-27](/posts/301-clojure-from-the-ground-up-welcome#comment-1693)
;;
L;;oving this and thanks for the encouraging intro ( i am gay), just reading the “Clojure data analisys cookbook” and this will help me to go trough the chapters.
;;
...
;;
N;;ice tute - but isn’t it a read-Eval-print-loop, not execute?
;;
!;;[epyf](https://www.gravatar.com/avatar/e9fbeb37bfd7d3c5b187e5d8e2f422a6?r=pg&s=96&d=identicon "epyf")
;;
e;;pyf on [2013-10-28](/posts/301-clojure-from-the-ground-up-welcome#comment-1697)
;;
W;;ell, women, LGBT and people of color can always take classes on Coursera. The quality there is high too, and they won’t get the impression of being “people with special teaching ”. Or flagships - want it or not.
;;
...
!;;[epyf](https://www.gravatar.com/avatar/e9fbeb37bfd7d3c5b187e5d8e2f422a6?r=pg&s=96&d=identicon "epyf")
;;
e;;pyf on [2013-10-28](/posts/301-clojure-from-the-ground-up-welcome#comment-1698)
;;
W;;ell, women, LGBT and people of color can always take classes on Coursera. The quality there is high too, and they won’t get the impression of being “people with special teaching ”. Or flagships - want it or not.
;;
I;; do understand that the tutorial is aimed at just anybody, but this is really the feeling I get when I read that paragraph (but I’m white and straight, that may be the reason)
...
;;
A;;ndreas on [2013-10-28](/posts/301-clojure-from-the-ground-up-welcome#comment-1700)
;;
H;;i Kyle,
;;
t;;he goals of this introduction you’re mentioning are just wonderful. I have nothing than deep respect for your openness, encouragement _and_ your profound technical skills! Please, just keep walking this path.
;;
B;;est, Andreas
;;
...
;;
N;;ext I will try to introduce Clojure to my 18 years old daughter and I think I’ll use your blog as starting point. So please continue your excellent blog.
;;
B;;r, Jarppe
;;
A;;phyr on [2013-10-28](/posts/301-clojure-from-the-ground-up-welcome#comment-1702)
;;
G;;ood catch, it is a mistake. Thanks M.
;;
!;;[Aphyr](https://www.gravatar.com/avatar/e145b50faf662e70c066b13c98921900?r=pg&s=96&d=identicon "Aphyr")
;;
A;;phyr on [2013-10-28](/posts/301-clojure-from-the-ground-up-welcome#comment-1703)
;;
...
;;
D;;efinitely a more abstract path, though. I’m trying to explain the core structure as deeply as possible without getting bogged down, but without losing sight of inductive learners either. We’ll see how it goes. :)
;;
!;;[Aphyr](https://www.gravatar.com/avatar/e145b50faf662e70c066b13c98921900?r=pg&s=96&d=identicon "Aphyr")
;;
!;;[Sergey](https://www.gravatar.com/avatar/3343705ce45837959960b7e9119143bf?r=pg&s=96&d=identicon "Sergey")
;;
S;;ergey on [2013-11-18](/posts/301-clojure-from-the-ground-up-welcome#comment-1743)
;;
H;;ello, Kyle! You are a man! Thanks a bunch for this guide, looking forward to next posts :)
;;
!;;[Adrian](https://www.gravatar.com/avatar/0629ec4252f7aaf7988b5770e73de4f5?r=pg&s=96&d=identicon "Adrian")
;;
A;;drian on [2013-11-20](/posts/301-clojure-from-the-ground-up-welcome#comment-1748)
;;
T;;hankyou for this article. I often find myself wallpapering over my sexuality in professional environments to avoid any “awkward looks”. It is important for queer coders to be visible to their peers and their lives celebrated equally. It put a smile on my face to read something that speaks to me both socially and technically :)
;;
...
;;
T;;hanks for both the article and statement of purpose! This is a great reference for learning clojure– especially since I’m completely new to lisps.
;;
!;;[Juan Manuel](https://www.gravatar.com/avatar/a839a1dce92696539e7e9475604ef310?r=pg&s=96&d=identicon "Juan Manuel")
;;
J;;uan Manuel on [2013-12-16](/posts/301-clojure-from-the-ground-up-welcome#comment-1780)
;;
W;;hy don’t you publish this as a book? The contents deserve it.
;;
...
;;
A;;hsen on [2014-01-05](/posts/301-clojure-from-the-ground-up-welcome#comment-1804)
;;
n;;ice tutorial, eagerly waiting for the next chapters.
;;
...
;;
A;;my Jenkins on [2014-03-25](/posts/301-clojure-from-the-ground-up-welcome#comment-1850)
;;
O;;ne of the most useful things for a beginner to do is to muck around with the command line. Thanks for this!
;;
...
;;
a;;nonymous on [2014-07-30](/posts/301-clojure-from-the-ground-up-welcome#comment-1942)
;;
>;; Is there a table of contents page that I’ve missed?
;;
R;;ight up top, at the tags list: [http://aphyr.com/tags/Clojure-from-the-ground-up](http://aphyr.com/tags/Clojure-from-the-ground-up). I’ll be reformatting all this work for the book, eventually, where there’ll be actual chapter headings, etc. Had a bunch of other stuff on my plate recently but starting to get back to writing on CFTGU this month. :)
;;
!;;[Mike](https://www.gravatar.com/avatar/f2b0c578d79b3b269a1d8327843732c1?r=pg&s=96&d=identicon "Mike")
;;
M;;ike on [2014-09-08](/posts/301-clojure-from-the-ground-up-welcome#comment-1952)
;;
T;;hanks for making this series! I’m a novice trying to learn distributed systems and clojure, this blog is awesome. Two quick suggestions for the getting started section:
;;
I;;i’m on Ubuntu 14.04 and this works for me `which javacc`
;;
...
;;
S;;am on [2014-10-21](/posts/301-clojure-from-the-ground-up-welcome#comment-1970)
;;
H;;i,
;;
...
;;
P;;eter on [2014-11-11](/posts/301-clojure-from-the-ground-up-welcome#comment-1982)
;;
W;;as going to use this resource until I saw the psychotic rants of the author. No way I would support someone with such hatred for other people. get over it - most coders are male, most people are straight. No reason to become a hate-monger.
;;
!;;[Kevin](https://www.gravatar.com/avatar/294de3557d9d00b3d2d8a1e6aab028cf?r=pg&s=96&d=identicon "Kevin")
;;
K;;evin on [2014-12-23](/posts/301-clojure-from-the-ground-up-welcome#comment-2012)
;;
...
;;
T;;hanks for this. You write well and the technical pace is perfect and easy to follow.
;;
!;;[kiquetal](https://www.gravatar.com/avatar/4488d62a8a69ac5cf6910ac0f57114f1?r=pg&s=96&d=identicon "kiquetal")
;;
k;;iquetal on [2015-01-14](/posts/301-clojure-from-the-ground-up-welcome#comment-2021)
;;
W;;hen will the book be available on github?. I love learning xD
;;
...
;;
V;;ery sad to see that you do not include very clever AI chatterbots as well. Alas, I must search elsewhere to learn Clojure
;;
!;;[Dave Cottlehuber](https://www.gravatar.com/avatar/5cca6682e26539ee49633032eb126fc3?r=pg&s=96&d=identicon "Dave Cottlehuber")
;;
D;;ave Cottlehuber on [2015-01-26](/posts/301-clojure-from-the-ground-up-welcome#comment-2052)
;;
T;;hanks for one of the clearest introductions resources for Clojure, and the Jepsen series are an excellent gateway drug to the diabolic and fascinating world of distributed systems.
;;
!;;[Weslley](https://www.gravatar.com/avatar/294de3557d9d00b3d2d8a1e6aab028cf?r=pg&s=96&d=identicon "Weslley")
;;
W;;eslley on [2015-03-05](/posts/301-clojure-from-the-ground-up-welcome#comment-2146)
;;
H;;i, this introduction saved my life in starting to learn Clojure. Congratulations for your effort! Thanks
;;
!;;[Max G. Faraday](https://www.gravatar.com/avatar/8dc1fe8960712ba2df7c3db90c4984dc?r=pg&s=96&d=identicon "Max G. Faraday")
;;
M;;ax G. Faraday on [2015-04-18](/posts/301-clojure-from-the-ground-up-welcome#comment-2301)
;;
H;;ey, Love this… Great, Exceptional body of work you have here. I am sure I am not the only one with this sentiment: THANK YOU. I have been coding for 24 years now and in Clojure for the last few. Thanks, really good job.
;;
!;;[Sed](https://www.gravatar.com/avatar/294de3557d9d00b3d2d8a1e6aab028cf?r=pg&s=96&d=identicon "Sed")
;;
S;;ed on [2016-01-03](/posts/301-clojure-from-the-ground-up-welcome#comment-2580)
;;
...
;;
B;;enjohn on [2016-01-05](/posts/301-clojure-from-the-ground-up-welcome#comment-2583)
;;
H;;i Aphyr, nice work.
;;
...
;;
I;;t’s stuff that still, sadly, needs saying. Not, I think, because people are necessarily bigoted, but because people just don’t appreciate how it is to live the life of others around them. It’s good to be reminded.
;;
!;;[Jack](https://www.gravatar.com/avatar/11a330aa3a8a116a6043b59b4d4ca20b?r=pg&s=96&d=identicon "Jack")
;;
J;;ack on [2016-04-04](/posts/301-clojure-from-the-ground-up-welcome#comment-2633)
;;
H;;y can’t I read something these days without touthing the LGBT shit? IT industry is sick.
;;
!;;[Jack](https://www.gravatar.com/avatar/11a330aa3a8a116a6043b59b4d4ca20b?r=pg&s=96&d=identicon "Jack")
;;
J;;ack on [2016-04-04](/posts/301-clojure-from-the-ground-up-welcome#comment-2634)
;;
D;;espite the LGBT syndrome, the article series is simply the best.
;;
!;;[JSteve](https://www.gravatar.com/avatar/dc117c26f9120a8ff3282c8505a168ac?r=pg&s=96&d=identicon "JSteve")
;;
J;;Steve on [2016-04-07](/posts/301-clojure-from-the-ground-up-welcome#comment-2635)
;;
...
;;
Y;;ou’re white and male and you spent a lot of time representing females and minorities, which to me makes it sound like YOU consider them second class citizens who are unable to represent themselves. Get off the high horse and write something that doesn’t make me want to barf. I don’t hate you or people like you, but I do hate all this victimization crap.
;;
!;;[Shayne O](https://www.gravatar.com/avatar/efb8f50e6ffe6cfbdc31f308a25606fc?r=pg&s=96&d=identicon "Shayne O")
;;
S;;hayne O on [2016-09-26](/posts/301-clojure-from-the-ground-up-welcome#comment-2699)
;;
G;;ood work on reaching out to women and other less represented groups. Having youngsters in my family interested in coding, a lot of the attitudes seems discouraging for the girls, so I think its great folks are making an effort to reach out.
;;
N;;ever mind the huffing and puffing troglodytes who weaze and moan everytime someone even suggests including folks often excluded in this industry, they are on the wrong side of history and they know it. Your doing the right thing Kyle. :)
;;
...
;;
W;;ithout this guide, where would one start with Clojure? I dont know and I dont want to entertain the thought.
;;
T;;hat being said, you do not need Leinengen to start with clojure. I was behind a corporate firewall teach myself clojure and could not access things like lein through the proxy.
;;
Y;;ou can just download clojure. And optionally download clojure-mode and inf-clojure from github and away you go!
;;
...
;;
I;;t is a very good one can you publish this one as a book?
;;
!;;[Daniel Carpenter](https://www.gravatar.com/avatar/3c18c38162620c52de5b1c79adab8746?r=pg&s=96&d=identicon "Daniel Carpenter")
;;
D;;aniel Carpenter on [2017-02-01](/posts/301-clojure-from-the-ground-up-welcome#comment-2739)
;;
T;;hank you so much for this. What a wonderful resource.
;;
!;;[Patrick](https://www.gravatar.com/avatar/3a9da6b673e0b31da07fe059d8485f78?r=pg&s=96&d=identicon "Patrick")
;;
P;;atrick on [2017-03-24](/posts/301-clojure-from-the-ground-up-welcome#comment-2801)
;;
T;;hank you for writing this guide, and for your introduction. Both are awesome.
;;
!;;[rosazon](https://www.gravatar.com/avatar/0c0e6cb66c4bb6e39e520e25136c3ea3?r=pg&s=96&d=identicon "rosazon")
;;
r;;osazon on [2017-03-26](/posts/301-clojure-from-the-ground-up-welcome#comment-2803)
;;
...
;;
M;;aia on [2017-09-11](/posts/301-clojure-from-the-ground-up-welcome#comment-2895)
;;
O;;oooph, some of the comments here. :-\\ I thought your intro was welcoming and personal, and I appreciated it. So far this guide is really helpful and clear, and the order is making more sense to me than other Clojure tutorials I’ve been looking at. Thank you for writing it!
;;
...
;;
I;;f you like to avoid as much as possible to clutter your base system with cruft, there’s an official Clojure Docker container ready to rock with everything you might need: [https://hub.docker.com/\_/clojure/](https://hub.docker.com/_/clojure/)
;;
!;;[Pankaj Doharey](https://www.gravatar.com/avatar/788814e1cf0b714c87b845ee840565e1?r=pg&s=96&d=identicon "Pankaj Doharey")
;;
P;;ankaj Doharey on [2018-07-29](/posts/301-clojure-from-the-ground-up-welcome#comment-2972)
;;
W;;ell Said “All it takes to be a good engineer, scientist, or mathematician is your curiosity, your passion, the right teaching material, and putting in the hours.”
;;
...
;;
G;;ary Rawlings on [2018-09-26](/posts/301-clojure-from-the-ground-up-welcome#comment-2986)
;;
...
;;
K;;en on [2018-11-12](/posts/301-clojure-from-the-ground-up-welcome#comment-2995)
;;
...
;;
W;;ALLACE: How are we going to get rid of racism until …?
;;
F;;REEMAN: Stop talking about it. I’m going to stop calling you a white man. And I’m going to ask you to stop calling me a black man. I know you as Mike Wallace. You know me as Morgan Freeman. You’re not going to say, “I know this white guy named Mike Wallace.” Hear what I’m saying?
;;
!;;[Tod](https://www.gravatar.com/avatar/f6a9ed4d0603452bd4266b1480526b01?r=pg&s=96&d=identicon "Tod")
;;
T;;od on [2018-11-28](/posts/301-clojure-from-the-ground-up-welcome#comment-2999)
;;
K;;yle, I don’t know. All the personal stuff at the beginning sort of cleared my mind. Then your essay was very clear. Have you read Frege?
;;
...
!;;[](https://www.gravatar.com/avatar/294de3557d9d00b3d2d8a1e6aab028cf?r=pg&s=96&d=identicon)
;;
a;;nonymous on [2021-07-12](/posts/301-clojure-from-the-ground-up-welcome#comment-3494)
;;
;;真的非常cool
;;
!;;[](https://www.gravatar.com/avatar/294de3557d9d00b3d2d8a1e6aab028cf?r=pg&s=96&d=identicon)
;;
a;;nonymous on [2021-08-03](/posts/301-clojure-from-the-ground-up-welcome#comment-3507)
;;
N;;ice
;;
!;;[Ahmad ](https://www.gravatar.com/avatar/294de3557d9d00b3d2d8a1e6aab028cf?r=pg&s=96&d=identicon "Ahmad ")
;;
A;;hmad on [2022-03-24](/posts/301-clojure-from-the-ground-up-welcome#comment-3711)
;;
T;;hanks for this guide. Nevermind the people getting furious about a really kind introductory paragraph to make some people that might have a rough time feel included. They could have just skipped that part if they didn’t like it. I think it’s refreshing to see something personal, makes the rest of the guide feel more human. I read your story too and it was touching.
;;
!;;[Andy](https://www.gravatar.com/avatar/18a887613be1838f47055e2c04d639e8?r=pg&s=96&d=identicon "Andy")
;;
A;;ndy on [2023-02-22](/posts/301-clojure-from-the-ground-up-welcome#comment-4254)
;;
A;;n opening paragraph about people being different? You should hear what my C# / Java colleagues think about me being a functional programmer, let alone programming in (dare we say it)… LISP. ;)
;;
P;;ost a Comment
=;;=============
;;
C;;omments are moderated. Links have `nofollow`. Seriously, spammers, give it a rest.
;;
P;;lease avoid writing anything here unless you're a computer. Captcha  This is also a trap: Comment
;;
N;;ame 
;;
...
C;;omment
;;
S;;upports [Github-flavored Markdown](https://guides.github.com/features/mastering-markdown/), including `[links](http://foo.com/)`, `*emphasis*`, `_underline_`, `` `code` ``, and `> blockquotes`. Use ` ```clj ` on its own line to start an (e.g.) Clojure code block, and ` ``` ` to end the block.    
;;
C;;opyright © 2023 Kyle Kingsbury.  
A;;lso on: [Mastodon](https://woof.group/@aphyr) and [Github](https://github.com/aphyr).
;;
